...
function Editcategory() {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/categorys/updatecategory/${obj.id}`,{
      headers:{
  ...
  const updateCategory = (e) => {
    ...
    axios.put(`http://localhost:5000/categorys/addcategorys/${obj.id}`, {
      categoryname: name,
      number: qty,
      status: status,
      // file:imagePreview
        ...
        <div className="p-6 bg-background rounded-lg shadow-md w-4/5 mt-24">
          ...
          <form >
            ...
            <div className="border rounded-lg p-4 bg-card">
                ...
                {image ? (
                  <img alt="preview" src={`http://localhost:5000/images/${image}`} className="rounded-md mb-2 w-20 h-20" />
                ) : (
